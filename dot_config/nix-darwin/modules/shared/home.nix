...

{
  ...
  home = {
    ...
    # entry is all it needs. ~/.zshenv is the one file every zsh start reads.
    #
    # mise shims prepend (its node must resolve ahead of nixpkgs'); ~/.local/bin
    # appends so it can't shadow a profile's tool. Both guards stop a nested shell
    # doubling PATH. fish sources this via fenv; bash gets it from extraInit. The
    # marker is what darwin.nix's extraInit checks, so a shell that ran one didn't
    # run the other. This replaces the file home-manager's zsh module would write.
    file.".zshenv".text = lib.concatLines [
      "# Generated by Home Manager; see modules/shared/home.nix for the why."
      "HM_ZSHENV_PATH_DONE=1"
      "case \":$PATH:\" in"
  ...
  programs = {
    ...
    zsh = {
      ...
      #
      # Shims go on PATH directly, not through `mise activate` (a shell hook):
      # a PATH entry also resolves in the non-interactive shells a harness spawns,
      # where zsh never reads .zshrc. Interactive, not profileExtra — a GUI-launched
      # harness starts non-login shells, where a login-only export is missing.
      initContent = ''
        export PATH="$HOME/.local/share/mise/shims:$PATH"
      '';
    };
